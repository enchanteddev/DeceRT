...
pub fn schedule(topology:HashMap<u32,Conf>) -> Result<(), String> {
    ...
    let mut cpus: HashMap<u32, CPU> = topology.iter().map(|entry|
    {
    ...
    loop {
        ...
        for task in &task_currently_scheduled {
            task.args.iter().for_each(|sensor| 
                sensor_bitmap.set(sensors_to_int[sensor], true
                ));
        ...
        // free up the sensors
        curr_task.args.iter().for_each(|sensor| 
            sensor_bitmap.set(sensors_to_int[sensor], false
            ));
        ...
        codewriter.append(CodeTask::FunctionCall(FunctionCall{
            fn_identifier: curr_task.name,
